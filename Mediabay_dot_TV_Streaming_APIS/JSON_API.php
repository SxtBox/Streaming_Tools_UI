...
/* SETTINGS */
$Playlist_Name = "MyVideo"; // PLAYLIST NAME
define("Streaming_PATH", "./Streams/"); // Streaming PATH
$streams_path = "Streams/";    // Streaming PATH
...
function get_files($dir, &$get_files = array()) {
  ...
  while ($item = readdir($handle)) {
    if ($item == '.' or $item == '..')
      continue;
    $Streaming_PATH = $dir.'/'.$item;
    if (is_dir($Streaming_PATH))
      get_files($Streaming_PATH, $get_files);
    else
      $get_files[] = $Streaming_PATH;
  }
  closedir($handle);
  return $get_files;
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
$i = 0;
$get_files = get_files(Streaming_PATH);
...
$stream_url = substr($item, strlen(Streaming_PATH) + 1); // TAKE OFF THE TOP PATH
...
$json_data [] = array(
	"id" => $ids++,
	"title" => slug_to_title($title),
	"file" => $stream_url,
	"description" => slug_to_title1($title),
	"slug" => (trim(($title))),
